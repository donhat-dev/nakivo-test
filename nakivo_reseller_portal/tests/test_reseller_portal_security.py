...
class TestResellerPortalSecurity(TransactionCase):
    ...
    def test_opportunity_reseller_computes_from_customer_reseller(self):
        lead = self.lead_model.create(
            {
                "name": "Computed Reseller Opportunity",
                "type": "opportunity",
                "partner_id": self.customer_a.id,
            }
        )

        self.assertEqual(lead.reseller_partner_id, self.reseller_a)
