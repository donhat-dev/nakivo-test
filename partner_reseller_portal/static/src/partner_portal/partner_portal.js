...
export class PartnerPortalRoot extends Component {
    static template = "partner_reseller_portal.PartnerPortalRoot";
    static props = {
        apiBasePath: String,
        csrfToken: String,
    };
    ...
    async deleteOpportunity(ev) {
        const opportunityId = Number(ev.currentTarget.dataset.id || 0);
        if (!opportunityId) {
            return;
        }
...
registry
    ...
    .add("partner_reseller_portal.PartnerPortalRoot", PartnerPortalRoot);
