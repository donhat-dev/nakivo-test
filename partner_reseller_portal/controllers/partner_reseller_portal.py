...
from odoo.addons.base_rest_api.exceptions import (
    PermissionDeniedException,
    ValidationException,
)
from odoo.addons.base_rest_api.handler import BaseRestHandler
...
class PartnerResellerPortalController(CustomerPortal, BaseRestHandler):
    ...
    @route(PORTAL_PAGE_ROUTE, type="http", auth="user", website=True, sitemap=False)
    def reseller_portal_page(self, **kwargs):
        self._ensure_portal_page_access()
        values = self._prepare_portal_layout_values()
        values.update(
            {
                "page_name": "partner_reseller_portal",
        ...
        return request.render(
            "partner_reseller_portal.portal_my_reseller_portal", values
        )
    ...
    def reseller_portal_opportunities(self, **kwargs):
